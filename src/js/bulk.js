...
const tagLabels = window.tagLabels; // shared mutable object ref
    ...
    Swal.fire({
      ...
      ids.forEach(id => {
        ...
        if (alreadyInTrash) {
          ...
          setTimeout(() => {
            card.remove();
            if (id && window.db && typeof window.BOARD_ID !== 'undefined') {
              window.db.collection(`boards/${window.BOARD_ID}/tasks`).doc(id).delete().catch(() => {});
            }
          }, 200);
        } else if (trashCol) {
          movedCount++;
      ...
      setTimeout(() => { if (window.saveChanges) saveChanges(true); }, 350);
      if (window.logActivity) {
        const author = window.currentUser?.displayName || window.currentUser?.email || 'User';
