...
(function () {
  ...
  function loadTemplates() {
    if (typeof db === 'undefined' || !window.BOARD_ID) { templateRow.style.display = 'none'; return; }
    window.db.collection(`boards/${window.BOARD_ID}/templates`).get()
      .then(snap => {
  ...
  function renderTemplateList() {
    ...
    _templates.forEach(tpl => {
      ...
      delBtn.addEventListener('click', e => {
        ...
        window.db.collection(`boards/${window.BOARD_ID}/templates`).doc(tpl.id).delete()
          .then(() => {
  ...
  addBtn.addEventListener('click', () => {
    ...
    if (_editingCard) {
      ...
      const _editAuthor = window.currentUser?.displayName || 'Someone';
      if (oldText !== text) {
    ...
    const authorName  = window.currentUser?.displayName || window.currentUser?.email || 'You';
    const authorPhoto = window.currentUser?.photoURL    || '';
    ...
    card.dataset.id             = window.db.collection(`boards/${window.BOARD_ID}/tasks`).doc().id;
    card.dataset.created        = new Date().toISOString().split('T')[0];
    card.dataset.createdByUid   = window.currentUser?.uid         || '';
    ...
    const createTL = buildTimeline([{ type: 'create', author: window.currentUser?.uid || '', text: 'Card Created', date: createdDate, ts: Date.now() }], { createOnly: true });
